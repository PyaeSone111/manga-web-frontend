...
export function Card11Classic({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-300 hover:border-cyan-500/40 hover:cosmic-glow h-full min-h-[140px]">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between p-3">
        ...
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className="inline-block px-1.5 py-0.5 rounded text-[9px] font-medium bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
            {manga.genre}
          </span>
        </div>
      </div>
...
export function Card12Cinematic({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative min-h-[160px] transition-all duration-300 hover:cosmic-glow-strong">
      <img
        src={manga.coverUrl || "/placeholder.svg"}
        alt={manga.title}
        ...
        <div className="flex flex-col gap-2 max-w-[65%]">
          ...
          <div className="flex items-center gap-4 mt-1">
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-cyan-400 font-bold">{manga.rating.toFixed(1)}</span>
            </div>
...
export function Card13Glass({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-2xl overflow-hidden glass border border-white/10 transition-all duration-300 hover:border-cyan-500/30 hover:cosmic-glow h-full min-h-[140px]">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between p-3 gap-1">
        <div>
          <h3 className="font-bold text-sm text-white line-clamp-1 leading-tight group-hover:text-cyan-400 transition-colors">
            {manga.title}
          </h3>
        ...
        <div className="flex flex-wrap gap-1">
          {manga.tags.map((tag) => (
            <span key={tag} className="px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-white/5 text-slate-300 border border-white/10">
              {tag}
            </span>
          ))}
        </div>
...
export function Card14Action({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl border border-slate-800 bg-slate-900/90 overflow-hidden transition-all duration-300 hover:border-cyan-500/40 h-full min-h-[140px]">
      <div className="relative flex-shrink-0 w-24 sm:w-28 overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between p-3">
        <div>
          <h3 className="font-bold text-sm text-slate-100 line-clamp-1 leading-tight group-hover:text-cyan-400 transition-colors">
            {manga.title}
          </h3>
          <p className="text-[11px] text-slate-400 mt-0.5">{manga.author}</p>
        ...
        <div className="flex items-center gap-2 mt-2">
          <button type="button" className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-cyan-500 text-slate-950 text-[11px] font-bold hover:bg-cyan-400 transition-colors">
            Read Now
            <ChevronRight size={12} />
          </button>
          <button type="button" className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800 text-slate-300 text-[11px] font-medium hover:bg-slate-700 transition-colors border border-slate-700">
            <Bookmark size={11} />
            Save
          </button>
        </div>
...
export function Card15Compact({ manga, rank }: { manga: MangaSeries; rank: number }) {
  return (
    <div className="group cursor-pointer flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-900/60 p-2.5 transition-all duration-300 hover:border-cyan-500/30 hover:bg-slate-900/90">
      <span className="text-lg font-black text-slate-700 group-hover:text-cyan-500/50 transition-colors w-6 text-center flex-shrink-0">
        {rank}
      </span>
...
export function Card16Neon({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl overflow-hidden border-2 border-cyan-500/20 bg-slate-900 transition-all duration-300 hover:border-cyan-400/50 hover:shadow-[0_0_30px_rgba(6,182,212,0.25)] h-full min-h-[140px]">
      <div className="relative flex-shrink-0 w-28 overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between p-3 border-l border-cyan-500/10">
        <div>
          ...
          <h3 className="font-bold text-sm text-cyan-50 line-clamp-1 leading-tight group-hover:text-cyan-400 transition-colors">
            {manga.title}
          </h3>
          <p className="text-[11px] text-slate-400 mt-0.5">{manga.author}</p>
        </div>
...
export function Card17Dense({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-300 hover:border-cyan-500/30 h-full min-h-[160px]">
      <div className="relative flex-shrink-0 w-28 overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
...
export function Card18Banner({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative min-h-[180px] transition-all duration-300 hover:cosmic-glow">
      <img
        src={manga.coverUrl || "/placeholder.svg"}
        alt={manga.title}
        className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 brightness-40"
      />
      ...
      <div className="relative z-10 flex h-full min-h-[180px] p-5 gap-4">
        ...
        <div className="flex flex-col justify-center gap-1.5 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-1">
            ...
            <span className="text-xs text-cyan-400 font-semibold">{manga.rating.toFixed(1)}</span>
            <div className="flex items-center gap-1 text-slate-400">
              <Eye size={12} />
              <span className="text-[11px]">{manga.views}</span>
            </div>
...
export function Card19Timeline({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-300 hover:border-cyan-500/30 h-full min-h-[120px]">
...
export function Card20Trending({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer flex rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-300 hover:border-emerald-500/30 hover:shadow-[0_0_20px_rgba(16,185,129,0.15)] h-full min-h-[140px]">
      <div className="relative flex-shrink-0 w-28 overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between p-3">
        <div>
          <h3 className="font-bold text-sm text-slate-100 line-clamp-1 leading-tight group-hover:text-emerald-400 transition-colors">
            {manga.title}
          </h3>
          <p className="text-[11px] text-slate-400 mt-0.5">{manga.author}</p>
