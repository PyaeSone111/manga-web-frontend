...
import { Star, BookOpen, Eye, Clock, Heart, Bookmark, TrendingUp, Flame, Award, Sparkles } from "lucide-react"
...
export function Card01Classic({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden transition-all duration-300 hover:border-cyan-500/40 hover:cosmic-glow flex flex-col h-full">
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <h3 className="font-semibold text-sm text-slate-100 line-clamp-2 leading-tight group-hover:text-cyan-400 transition-colors">
          {manga.title}
        </h3>
        <p className="text-xs text-slate-400 line-clamp-1">{manga.author}</p>
        <div className="flex items-center gap-2 mt-auto pt-1">
          <StarRating rating={manga.rating} />
          <span className="text-[10px] text-slate-500">{manga.rating.toFixed(1)}</span>
        </div>
        <span className="inline-block w-fit px-2 py-0.5 rounded-md text-[10px] font-medium bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
          {manga.genre}
        </span>
      </div>
    </div>
...
export function Card02Overlay({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative aspect-[2/3] transition-all duration-300 hover:cosmic-glow-strong">
      <img
        src={manga.coverUrl || "/placeholder.svg"}
        alt={manga.title}
      ...
      <div className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-slate-950/70 backdrop-blur-sm px-2 py-1">
        ...
        <span className="text-[11px] font-semibold text-cyan-400">{manga.rating.toFixed(1)}</span>
      </div>
...
export function Card03Neon({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative flex flex-col h-full border-2 border-cyan-500/20 bg-slate-900 transition-all duration-300 hover:border-cyan-400/60 hover:shadow-[0_0_30px_rgba(6,182,212,0.3)]">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 bg-slate-950/80 backdrop-blur-sm rounded-full px-2 py-1">
            <Star size={10} className="fill-cyan-400 text-cyan-400" />
            <span className="text-[10px] text-cyan-400 font-semibold">{manga.rating.toFixed(1)}</span>
          </div>
        </div>
      </div>
      <div className="p-3 flex flex-col gap-1 flex-1 border-t border-cyan-500/10">
        <h3 className="font-bold text-sm text-cyan-50 line-clamp-2 leading-tight group-hover:text-cyan-400 transition-colors">
          {manga.title}
        </h3>
        <p className="text-[11px] text-slate-400">{manga.author}</p>
        <div className="flex items-center gap-1.5 mt-auto pt-1.5">
          {manga.tags.slice(0, 2).map((tag) => (
            <span key={tag} className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
              {tag}
            </span>
          ))}
        </div>
...
export function Card04Minimal({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-2xl overflow-hidden flex flex-col h-full bg-slate-900/50 transition-all duration-300 hover:bg-slate-900/80">
      ...
      <div className="px-3 py-3 flex flex-col gap-0.5 flex-1">
        <h3 className="font-semibold text-sm text-slate-100 line-clamp-1 group-hover:text-cyan-400 transition-colors">
          {manga.title}
        </h3>
        <p className="text-[11px] text-slate-500">{manga.author}</p>
        <div className="flex items-center gap-1 mt-auto pt-1">
          <Star size={10} className="fill-cyan-400 text-cyan-400" />
          <span className="text-[11px] text-slate-400 font-medium">{manga.rating.toFixed(1)}</span>
        </div>
      </div>
    </div>
  )
...
export function Card05Badges({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden flex flex-col h-full transition-all duration-300 hover:border-violet-500/40">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-2">
          <div className="flex flex-wrap gap-1">
            {manga.tags.map((tag) => (
              <span key={tag} className="px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-slate-950/80 backdrop-blur-sm text-slate-300 border border-slate-700/50">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <h3 className="font-semibold text-sm text-slate-100 line-clamp-2 leading-tight group-hover:text-violet-400 transition-colors">
          {manga.title}
        </h3>
        <p className="text-xs text-slate-400">{manga.author}</p>
        <div className="flex items-center gap-2 mt-auto pt-1">
          <StarRating rating={manga.rating} />
          <span className="text-[10px] text-slate-500">({manga.ratingCount.toLocaleString()})</span>
        </div>
      </div>
    </div>
  )
...
export function Card06Stats({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl border border-slate-800 bg-slate-900/90 overflow-hidden flex flex-col h-full transition-all duration-300 hover:border-cyan-500/30 hover:cosmic-glow">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
        ...
        <div className="absolute bottom-2 left-2 right-2 grid grid-cols-3 gap-1">
          ...
          <div className="flex flex-col items-center bg-slate-950/80 backdrop-blur-sm rounded-lg py-1.5 px-1">
            ...
            <span className="text-[9px] text-cyan-400 font-semibold">{manga.rating.toFixed(1)}</span>
          </div>
        </div>
      </div>
      <div className="p-3 flex flex-col gap-1 flex-1">
        <h3 className="font-bold text-sm text-slate-100 line-clamp-2 leading-tight group-hover:text-cyan-400 transition-colors">
          {manga.title}
        </h3>
        <p className="text-[11px] text-slate-400">{manga.author}</p>
...
export function Card07Glass({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-2xl overflow-hidden relative aspect-[2/3] transition-all duration-300 hover:cosmic-glow-strong">
      <img
        src={manga.coverUrl || "/placeholder.svg"}
        alt={manga.title}
      ...
      <div className="absolute top-3 right-3">
        <div className="flex items-center gap-1 glass rounded-full px-2.5 py-1 border border-white/10">
          ...
          <span className="text-[11px] font-bold text-white">{manga.rating.toFixed(1)}</span>
        </div>
      </div>
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <div className="glass rounded-xl border border-white/10 p-3 flex flex-col gap-1.5">
...
export function Card08Accent({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden flex flex-col h-full bg-slate-900 border-l-4 border-l-cyan-500 border border-slate-800 transition-all duration-300 hover:border-cyan-500/40 hover:bg-slate-900/90">
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-bold text-sm text-slate-100 line-clamp-2 leading-tight group-hover:text-cyan-400 transition-colors flex-1">
            {manga.title}
          </h3>
        ...
        <div className="flex items-center gap-2 mt-auto pt-1">
          <div className="flex items-center gap-1">
            <Star size={10} className="fill-cyan-400 text-cyan-400" />
            <span className="text-[11px] text-cyan-400 font-semibold">{manga.rating.toFixed(1)}</span>
          </div>
...
export function Card09Cinematic({ manga }: { manga: MangaSeries }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative flex flex-col h-full bg-slate-950 transition-all duration-300 hover:cosmic-glow">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
      ...
      <div className="p-3 flex flex-col gap-1 flex-1 bg-slate-950">
        <h3 className="font-bold text-sm text-white line-clamp-2 leading-tight tracking-wide uppercase group-hover:text-cyan-400 transition-colors">
          {manga.title}
        </h3>
...
export function Card10Rank({ manga, rank }: { manga: MangaSeries; rank: number }) {
  return (
    <div className="group cursor-pointer rounded-xl overflow-hidden relative flex flex-col h-full border border-slate-800 bg-slate-900/80 transition-all duration-300 hover:border-cyan-500/40 hover:cosmic-glow">
      <div className="relative w-full aspect-[2/3] overflow-hidden">
        <img
          src={manga.coverUrl || "/placeholder.svg"}
          alt={manga.title}
        ...
        <div className="absolute -bottom-2 -left-1">
          <span
            ...
            style={{
              WebkitTextStroke: "2px rgba(6, 182, 212, 0.6)",
              color: "transparent",
